...
public class IRStatement {
    ...
    //统计每个conf 有几个相同
    public List<diagSlicer.AnalysisCount> AnalysisCount;
    ...
    public IRStatement(StatementWithInstructionIndex stmt) {
        ...
        //check stmt not null
        ConfUtils.checkNotNull(this.s);
        ConfUtils.checkNotNull(this.ssa);

        ConfUtils.checkTrue(this.instructionIndex > -1);
        ConfUtils.checkTrue(this.bcIndex > -1);
    }
    ...
    public void Analysis(List<ConfEntity> confList) {
        ...
        System.out.println("confSize::"+ConfSize);
        int CountSize = this.eachCount.size();
        System.out.println("CoutSize::"+CountSize);
